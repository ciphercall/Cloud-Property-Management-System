...
namespace Cloud_Property.Controllers
{
    public class TransactionController : AppController
    {
        ...
        public JsonResult Update_SelectTransNo(Int64 changedtxt, string type)
        {
            ...
            if (type == "MREC")
            {
                var ans1 = (from n in db.GlAcchartDbSet where n.COMPID == compid && (n.HEADCD == matchingHead1 || n.HEADCD == matchingHead2) select new { n.ACCOUNTCD, n.ACCOUNTNM }).OrderBy(x => x.ACCOUNTNM);
                foreach (var x in ans1)
                {
            ...
            {
                var ans2 = (from n in db.GlAcchartDbSet where n.COMPID == compid && (n.HEADCD != matchingHead1 && n.HEADCD != matchingHead2) select new { n.ACCOUNTCD, n.ACCOUNTNM }).OrderBy(x => x.ACCOUNTNM);
                foreach (var x in ans2)
                {
                    Debitlist.Add(new SelectListItem { Text = x.ACCOUNTNM, Value = Convert.ToString(x.ACCOUNTCD) });
                }
            ...
            {
                var ans3 = (from n in db.GlAcchartDbSet where n.COMPID == compid && (n.HEADCD == matchingHead1 || n.HEADCD == matchingHead2) select new { n.ACCOUNTCD, n.ACCOUNTNM }).OrderBy(x => x.ACCOUNTNM);
                foreach (var x in ans3)
                {
                    Debitlist.Add(new SelectListItem { Text = x.ACCOUNTNM, Value = Convert.ToString(x.ACCOUNTCD) });
                }
            ...
            {
                var ans4 = (from n in db.GlAcchartDbSet where n.COMPID == compid && (n.HEADCD != matchingHead1 && n.HEADCD != matchingHead2) select new { n.ACCOUNTCD, n.ACCOUNTNM }).OrderBy(x => x.ACCOUNTNM);
                foreach (var x in ans4)
                {
                    Debitlist.Add(new SelectListItem { Text = x.ACCOUNTNM, Value = Convert.ToString(x.ACCOUNTCD) });
                }
            ...
            var result = new { keyid = primaryid, For = transfor, date = chequedate, costpool = costpoolid, mode = transmode, firstdebittext = debitaccount, 
                debitCD = debitcd, debit = Debitlist, firstcredittext = creditaccount, creditCD = creditcd, credit = Creditlist, cheque = chequeno,
                ChequeDate = chequedate,remarks = remarks,amount = amount,INSUSERID = insertUserId,INSTIME = inserttime,INSIPNO = insertIpno,INSLTUDE = insltude
            };
