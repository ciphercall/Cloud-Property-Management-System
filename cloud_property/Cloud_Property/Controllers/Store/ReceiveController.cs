...
{
    public class ReceiveController : AppController       
    {
        //Datetime formet
        ...
        public ActionResult IndexPost(PageModel model, string command)
        {
            if (command == "Add")
            {
                ...
                var checkPermission = from role in db.AslRoleDbSet
                                      where role.USERID == loggedUserID && role.CONTROLLERNAME == "Receive" && role.ACTIONNAME == "Index"
                ...
                model.StkTrans.TRANSTP = "IREC";
                model.StkTrans.STORETO = model.StkStore.STOREID;
                //model.StkTrans.PSID = model.GlAcchart.ACCOUNTCD;


                var res = db.StkTransDbSet.Where(a => a.TRANSNO == model.StkTrans.TRANSNO && a.TRANSTP == "IREC" && a.TRANSYY == model.StkTrans.TRANSYY && a.COMPID == compid).Count(a => a.ITEMID == model.StkTrans.ITEMID) == 0;

                if (res == true)
                {
                    var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSTP == "IREC" && x.TRANSYY == model.StkTrans.TRANSYY && x.COMPID == compid)
                                  .Max(o => o.ITEMSL);
                    var transno_Max = db.StkTransDbSet.Where(x => x.TRANSYY == model.StkTrans.TRANSYY && x.TRANSTP == "IREC" && x.COMPID == compid)
                        .Max(s => s.TRANSNO);
                ...
                else
                {
                    var result = (from n in db.StkTransDbSet
                                  where n.TRANSNO == model.StkTrans.TRANSNO &&
                                        ...
                                        n.TRANSYY == model.StkTrans.TRANSYY && n.TRANSTP == "IREC"
                                  select new
                                  {
            ...
            else if (command == "Save")
            {
                ...
                model.StkTransmst.TRANSTP = "IREC";
        ...
        public ActionResult OrderDelete(Int64 tid, Int64 orderNo, DateTime Date, Int64 Year, Int64 itemsl, PageModel model)
        {
            ...
            var result = (from t in db.StkTransDbSet
                          where t.COMPID == compid && t.TRANSNO == orderNo && t.TRANSYY == Year && t.TRANSTP == "IREC"
            ...
            {
                var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSYY == Year && x.TRANSTP == "IREC" && x.COMPID == compid)
                            .Max(o => o.ITEMSL);
        ...
        public ActionResult EditOrder(PageModel model, string command)
        {
            if (command == "Add")
            {
                ...
                var res = db.StkTransDbSet.Where(a => a.TRANSDT == model.StkTrans.TRANSDT && a.TRANSNO == model.StkTrans.TRANSNO && a.TRANSTP == "IREC" && a.COMPID == compid).Count(a => a.ITEMID == model.StkTrans.ITEMID) == 0;
                if (res == true)
                {
                    //Permission Check
                    Int64 loggedUserID = Convert.ToInt64(System.Web.HttpContext.Current.Session["loggedUserID"]);
                    ...
                    var get_TRANSDT_TRANSYY = (from m in db.StkTransDbSet
                                               where m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSTP == "IREC"
                    ...
                    var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSTP == "IREC" && x.COMPID == compid)
                               .Max(o => o.ITEMSL);
                    var transno_Max = db.StkTransDbSet.Where(x => x.TRANSYY == model.StkTrans.TRANSYY && x.TRANSTP == "IREC" && x.COMPID == compid)
                        .Max(s => s.TRANSNO);
                    ...
                    else
                    {
                        ...
                        var transMst = (from m in db.StkTransmstDbSet
                                        where m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSDT == model.StkTrans.TRANSDT && m.TRANSTP == "IREC"
                ...
                else
                {
                    var result = (from n in db.StkTransDbSet
                                  where n.TRANSNO == model.StkTrans.TRANSNO &&
                                        n.COMPID == compid &&
                                        n.ITEMID == model.StkTrans.ITEMID &&
                                        n.TRANSDT == model.StkTrans.TRANSDT && n.TRANSTP == "IREC"
                                  select new
                                  {
                                      transPID = n.STK_TRANS_ID,
                                      sl = n.ITEMSL,
            ...
            else if (command == "Save")
            {
                ...
                var findTransNO = (from n in db.StkTransmstDbSet
                                   where n.TRANSNO == model.StkTrans.TRANSNO && n.COMPID == model.StkTrans.COMPID &&
                          n.TRANSYY == model.StkTransmst.TRANSYY && n.TRANSTP == "IREC"
                                   select n).ToList();
                if (findTransNO.Count != 0)
                {
            ...
            else if (command == "search")
            {
                ...
                var transMst = (from m in db.StkTransmstDbSet
                                where m.TRANSDT == model.StkTrans.TRANSDT && m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSTP == "IREC"
        ...
        public ActionResult EditOrderDelete(Int64 tid, Int64 orderNo, DateTime Date, Int64 Year, Int64 itemsl, PageModel model)
        {
            ...
            var result = (from t in db.StkTransDbSet
                          where t.COMPID == compid && t.TRANSNO == orderNo && t.TRANSDT == Date && t.TRANSYY == Year && t.TRANSTP == "IREC"
                          select t).ToList();
            //Minimum RmsTrans Table MemoNO delete then RmsTransMst tabel data delete(key-> memoNO, compid)
            if (result.Count == 0)
            {
                var searchRmsTransMst = (from n in db.StkTransmstDbSet where n.COMPID == compid && n.TRANSNO == orderNo && n.TRANSDT == Date && n.TRANSYY == Year && n.TRANSTP == "IREC" select n).ToList();
            ...
            //Remarks pass the value.
            var transMst = (from m in db.StkTransmstDbSet
                            where m.COMPID == compid && m.TRANSNO == orderNo && m.TRANSDT == Date && m.TRANSYY == Year && m.TRANSTP == "IREC"
                            select new { m.STK_TRANSMST_ID, m.REMARKS }).ToList();
            if (transMst.Count != 0)
            {
                foreach (var a in transMst)
            ...
            var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSDT == Date && x.COMPID == compid && x.TRANSYY == Year && x.TRANSTP == "IREC").Max(o => o.ITEMSL);
            model.StkTrans.TRANSNO = orderNo;
            model.StkTrans.ITEMSL = sid;
            model.StkTrans.TRANSDT = Date;
            model.StkTrans.TRANSYY = Year;
        ...
        public JsonResult DateChanged(string theDate)
        {
            ...
            var transres = (from n in db.StkTransDbSet
                            where n.TRANSDT == dd && n.COMPID == compid && n.TRANSTP == "IREC"
                            select new
                            {
                                n.TRANSNO
                            }
