...
namespace Cloud_Property.Controllers
{
    public class IssueController : AppController
    {
        ...
        public void Insert_StkTransMst_LogData(STK_TRANSMST aStkTransmst)
        {
            ...
            string storeName = "";
            var findStoreName = (from n in db.StkStoreDbSet where n.COMPID == compid && n.STOREID == aStkTransmst.STOREFR select n).ToList();
        ...
        public void Delete_STK_TransMst_DELETE(STK_TRANSMST stkTransmst)
        {
            ...
            string storeName = "";
            var findStoreName = (from n in db.StkStoreDbSet where n.COMPID == compid && n.STOREID == stkTransmst.STOREFR select n).ToList();
            foreach (var x in findStoreName)
            {
                storeName = x.STORENM.ToString();
            }
        ...
        public ActionResult IndexPost(PageModel model, string command)
        {
            if (command == "Add")
            {
                ...
                var checkPermission = from role in db.AslRoleDbSet
                                      where role.USERID == loggedUserID && role.CONTROLLERNAME == "Issue" && role.ACTIONNAME == "Index"
                ...
                if (model.StkStore.STOREID == null && model.StkTrans.TRANSDT == null)
                {
                ...
                model.StkTrans.TRANSTP = "IISS";
                model.StkTrans.STOREFR = model.StkStore.STOREID;
                //model.StkTrans.PSID = model.GlAcchart.ACCOUNTCD;


                var res = db.StkTransDbSet.Where(a => a.TRANSNO == model.StkTrans.TRANSNO && a.TRANSTP == "IISS" && a.TRANSYY == model.StkTrans.TRANSYY && a.COMPID == compid).Count(a => a.ITEMID == model.StkTrans.ITEMID) == 0;

                if (res == true)
                {
                    var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSTP == "IISS" && x.TRANSYY == model.StkTrans.TRANSYY && x.COMPID == compid)
                                  .Max(o => o.ITEMSL);
                    var transno_Max = db.StkTransDbSet.Where(x => x.TRANSYY == model.StkTrans.TRANSYY && x.TRANSTP == "IISS" && x.COMPID == compid)
                        .Max(s => s.TRANSNO);
                    ...
                    if (model.StkTrans.TRANSNO == null)
                    {
                        ...
                        //TempData["ACCOUNTCD"] = model.GlAcchart.ACCOUNTCD;
                        TempData["transdate"] = model.StkTrans.TRANSDT;
                ...
                else
                {
                    var result = (from n in db.StkTransDbSet
                                  where n.TRANSNO == model.StkTrans.TRANSNO &&
                                        ...
                                        n.TRANSYY == model.StkTrans.TRANSYY && n.TRANSTP == "IISS"
                                  select new
                                  {
            ...
            else if (command == "Save")
            {
                ...
                model.StkTransmst.TRANSTP = "IISS";
                model.StkTransmst.STOREFR = model.StkStore.STOREID;
                //model.StkTransmst.PSID = model.GlAcchart.ACCOUNTCD;
        ...
        public ActionResult OrderDelete(Int64 tid, Int64 orderNo, DateTime Date, Int64 Year, Int64 itemsl, PageModel model)
        {
            ...
            var result = (from t in db.StkTransDbSet
                          where t.COMPID == compid && t.TRANSNO == orderNo && t.TRANSYY == Year && t.TRANSTP == "IISS"
                          select new { t.TRANSYY, t.STOREFR, t.PSID, t.TRANSDT }
             ).Distinct().ToList();
            ...
            foreach (var n in result)
            {
                ...
                model.StkStore.STOREID = n.STOREFR;
                //model.GlAcchart.ACCOUNTCD = n.PSID;
                TempData["STOREID"] = model.StkStore.STOREID;
                //TempData["ACCOUNTCD"] = model.GlAcchart.ACCOUNTCD;
            ...
            {
                var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSYY == Year && x.TRANSTP == "IISS" && x.COMPID == compid)
                            .Max(o => o.ITEMSL);
        ...
        public ActionResult OrderUpdate(Int64 tid, Int64 orderNo, DateTime Date, Int64 Year, Int64 itemsl, Int64 itemId, PageModel model)
        {
            ...
            model.StkStore.STOREID = model.StkTrans.STOREFR;
            //model.GlAcchart.ACCOUNTCD = model.StkTrans.PSID;
        ...
        public ActionResult EditOrder(PageModel model, string command)
        {
            if (command == "Add")
            {
                ...
                var res = db.StkTransDbSet.Where(a => a.TRANSDT == model.StkTrans.TRANSDT && a.TRANSNO == model.StkTrans.TRANSNO && a.TRANSTP == "IISS" && a.COMPID == compid).Count(a => a.ITEMID == model.StkTrans.ITEMID) == 0;
                if (res == true)
                {
                    //Permission Check
                    Int64 loggedUserID = Convert.ToInt64(System.Web.HttpContext.Current.Session["loggedUserID"]);
                    ...
                    var get_TRANSDT_TRANSYY = (from m in db.StkTransDbSet
                                               where m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSTP == "IISS"
                    ...
                    var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSTP == "IISS" && x.COMPID == compid)
                               .Max(o => o.ITEMSL);
                    var transno_Max = db.StkTransDbSet.Where(x => x.TRANSYY == model.StkTrans.TRANSYY && x.TRANSTP == "IISS" && x.COMPID == compid)
                        .Max(s => s.TRANSNO);
                    ...
                    else
                    {
                        ...
                        var transMst = (from m in db.StkTransmstDbSet
                                        where m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSDT == model.StkTrans.TRANSDT && m.TRANSTP == "IISS"
                ...
                else
                {
                    var result = (from n in db.StkTransDbSet
                                  where n.TRANSNO == model.StkTrans.TRANSNO &&
                                        n.COMPID == compid &&
                                        n.ITEMID == model.StkTrans.ITEMID &&
                                        n.TRANSDT == model.StkTrans.TRANSDT && n.TRANSTP == "IISS"
                                  select new
                                  {
                                      transPID = n.STK_TRANS_ID,
                                      sl = n.ITEMSL,
            ...
            else if (command == "Save")
            {
                ...
                var findTransNO = (from n in db.StkTransmstDbSet
                                   where n.TRANSNO == model.StkTrans.TRANSNO && n.COMPID == model.StkTrans.COMPID &&
                          n.TRANSYY == model.StkTransmst.TRANSYY && n.TRANSTP == "IISS"
                                   select n).ToList();
                if (findTransNO.Count != 0)
                {
                    foreach (STK_TRANSMST StkTransmst in findTransNO)
                    {
                        ...
                        StkTransmst.TRANSTP = "IISS";

                        StkTransmst.STOREFR = model.StkStore.STOREID;
                        //StkTransmst.PSID = model.GlAcchart.ACCOUNTCD;
            ...
            else if (command == "search")
            {
                ...
                var transMst = (from m in db.StkTransmstDbSet
                                where m.TRANSDT == model.StkTrans.TRANSDT && m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSTP == "IISS"
                                select new { m.STK_TRANSMST_ID, m.TOTAMT, m.DISCRTG, m.DISCAMTG, m.TOTGROSS, m.VATRT, m.VATAMT, m.OTCAMT, m.TOTNET, m.CASHAMT, m.CREDITAMT, m.REMARKS, m.STOREFR, m.PSID }).ToList();
                ...
                if (transMst != null)
                {
                    foreach (var a in transMst)
                    {
                        ...
                        model.StkStore.STOREID = a.STOREFR;
                        // model.GlAcchart.ACCOUNTCD = a.PSID;
                ...
                var stk_Trans = (from m in db.StkTransDbSet
                                 where m.TRANSDT == model.StkTrans.TRANSDT && m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSTP == "IISS"
                                 select new { m.STOREFR, m.PSID, m.TRANSYY }).ToList();
                foreach (var s in stk_Trans)
                {
                    model.StkStore.STOREID = s.STOREFR;
                    //model.GlAcchart.ACCOUNTCD = s.PSID;
        ...
        public ActionResult EditOrderDelete(Int64 tid, Int64 orderNo, DateTime Date, Int64 Year, Int64 itemsl, PageModel model)
        {
            ...
            var result = (from t in db.StkTransDbSet
                          where t.COMPID == compid && t.TRANSNO == orderNo && t.TRANSDT == Date && t.TRANSYY == Year && t.TRANSTP == "IISS"
                          select t).ToList();
            //Minimum RmsTrans Table MemoNO delete then RmsTransMst tabel data delete(key-> memoNO, compid)
            if (result.Count == 0)
            {
                var searchRmsTransMst = (from n in db.StkTransmstDbSet where n.COMPID == compid && n.TRANSNO == orderNo && n.TRANSDT == Date && n.TRANSYY == Year && n.TRANSTP == "IISS" select n).ToList();
            ...
            //Remarks pass the value.
            var transMst = (from m in db.StkTransmstDbSet
                            where m.COMPID == compid && m.TRANSNO == orderNo && m.TRANSDT == Date && m.TRANSYY == Year && m.TRANSTP == "IISS"
                            select new { m.STK_TRANSMST_ID, m.REMARKS }).ToList();
            if (transMst.Count != 0)
            {
                foreach (var a in transMst)
            ...
            var stk_Trans = (from m in db.StkTransDbSet
                             where m.COMPID == compid && m.TRANSNO == orderNo && m.TRANSDT == Date && m.TRANSYY == Year && m.TRANSTP == "IISS"
                             select new { m.STOREFR, m.PSID }).ToList();
            foreach (var s in stk_Trans)
            {
                model.StkStore.STOREID = s.STOREFR;
                //model.GlAcchart.ACCOUNTCD = s.PSID;
            ...
            var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSDT == Date && x.COMPID == compid && x.TRANSYY == Year && x.TRANSTP == "IISS").Max(o => o.ITEMSL);
            model.StkTrans.TRANSNO = orderNo;
            model.StkTrans.ITEMSL = sid;
            model.StkTrans.TRANSDT = Date;
            model.StkTrans.TRANSYY = Year;
        ...
        public JsonResult DateChanged(string theDate)
        {
            ...
            var transres = (from n in db.StkTransDbSet
                            where n.TRANSDT == dd && n.COMPID == compid && n.TRANSTP == "IISS"
                            select new
                            {
                                n.TRANSNO
                            }
