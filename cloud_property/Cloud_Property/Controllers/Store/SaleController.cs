    ...
    public class SaleController : AppController
    {
        ...
        public ActionResult IndexPost(PageModel model, string command)
        {
            if (command == "Add")
            {
                ...
                model.StkTrans.TRANSTP = "SALE";
                model.StkTrans.STOREFR = model.StkStore.STOREID;
                model.StkTrans.PSID = model.AGL_acchart.ACCOUNTCD;


                var res = db.StkTransDbSet.Where(a => a.TRANSNO == model.StkTrans.TRANSNO && a.TRANSTP == "SALE" && a.TRANSYY == model.StkTrans.TRANSYY && a.COMPID == compid).Count(a => a.ITEMID == model.StkTrans.ITEMID) == 0;

                if (res == true)
                {
                    var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSTP == "SALE" && x.TRANSYY == model.StkTrans.TRANSYY && x.COMPID == compid)
                                  .Max(o => o.ITEMSL);
                    var transno_Max = db.StkTransDbSet.Where(x => x.TRANSYY == model.StkTrans.TRANSYY && x.TRANSTP == "SALE" && x.COMPID == compid)
                        .Max(s => s.TRANSNO);
                ...
                else
                {
                    var result = (from n in db.StkTransDbSet
                                  where n.TRANSNO == model.StkTrans.TRANSNO &&
                                        ...
                                        n.TRANSYY == model.StkTrans.TRANSYY && n.TRANSTP == "SALE"
                                  select new
                                  {
            ...
            else if (command == "Save")
            {
                ...
                model.StkTransmst.TRANSTP = "SALE";
            ...
            else if (command == "A4" || command=="D/C")
            {
                if (model.StkTransmst.TOTNET == 0)
                {
                    ViewBag.addItemList = "Please Add item list!";
                ...
                aPageModel.StkTransmst.TRANSTP = "SALE";
        ...
        public ActionResult OrderDelete(Int64 tid, Int64 orderNo, DateTime Date, Int64 Year, Int64 itemsl, PageModel model)
        {
            ...
            var result = (from t in db.StkTransDbSet
                          where t.COMPID == compid && t.TRANSNO == orderNo && t.TRANSYY == Year && t.TRANSTP == "SALE"
            ...
            {
                var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSYY == Year && x.TRANSTP == "SALE" && x.COMPID == compid)
                            .Max(o => o.ITEMSL);
        ...
        public ActionResult EditOrder(PageModel model, string command)
        {
            if (command == "Add")
            {
                ...
                var res = db.StkTransDbSet.Where(a => a.TRANSDT == model.StkTrans.TRANSDT && a.TRANSNO == model.StkTrans.TRANSNO && a.TRANSTP == "SALE" && a.COMPID == compid).Count(a => a.ITEMID == model.StkTrans.ITEMID) == 0;
                if (res == true)
                {
                    //Permission Check
                    Int64 loggedUserID = Convert.ToInt64(System.Web.HttpContext.Current.Session["loggedUserID"]);
                    ...
                    var get_TRANSDT_TRANSYY = (from m in db.StkTransDbSet
                                               where m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSTP == "SALE"
                    ...
                    var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSTP == "SALE" && x.COMPID == compid)
                               .Max(o => o.ITEMSL);
                    var transno_Max = db.StkTransDbSet.Where(x => x.TRANSYY == model.StkTrans.TRANSYY && x.TRANSTP == "SALE" && x.COMPID == compid)
                        .Max(s => s.TRANSNO);
                    ...
                    else
                    {
                        ...
                        var transMst = (from m in db.StkTransmstDbSet
                                        where m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSDT == model.StkTrans.TRANSDT && m.TRANSTP == "SALE"
                ...
                else
                {
                    var result = (from n in db.StkTransDbSet
                                  where n.TRANSNO == model.StkTrans.TRANSNO &&
                                        n.COMPID == compid &&
                                        n.ITEMID == model.StkTrans.ITEMID &&
                                        n.TRANSDT == model.StkTrans.TRANSDT && n.TRANSTP == "SALE"
                                  select new
                                  {
                                      transPID = n.STK_TRANS_ID,
                                      sl = n.ITEMSL,
            ...
            else if (command == "Save")
            {
                ...
                var findTransNO = (from n in db.StkTransmstDbSet
                                   where n.TRANSNO == model.StkTrans.TRANSNO && n.COMPID == model.StkTrans.COMPID &&
                          n.TRANSYY == model.StkTransmst.TRANSYY && n.TRANSTP == "SALE"
                                   select n).ToList();
                if (findTransNO.Count != 0)
                {
            ...
            else if (command == "search")
            {
                ...
                var transMst = (from m in db.StkTransmstDbSet
                                where m.TRANSDT == model.StkTrans.TRANSDT && m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSTP == "SALE"
        ...
        public ActionResult EditOrderDelete(Int64 tid, Int64 orderNo, DateTime Date, Int64 Year, Int64 itemsl, PageModel model)
        {
            ...
            if (result.Count == 0)
            {
                var searchRmsTransMst = (from n in db.StkTransmstDbSet where n.COMPID == compid && n.TRANSNO == orderNo && n.TRANSDT == Date && n.TRANSYY == Year && n.TRANSTP == "SALE" select n).ToList();
            ...
            var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSDT == Date && x.COMPID == compid && x.TRANSYY == Year && x.TRANSTP == "SALE").Max(o => o.ITEMSL);
            model.StkTrans.TRANSNO = orderNo;
            model.StkTrans.ITEMSL = sid;
            model.StkTrans.TRANSDT = Date;
            model.StkTrans.TRANSYY = Year;
        ...
        public JsonResult ItemNameChanged(string changedText)
        {
            ...
            string discount = "";
            decimal qty = 0;
            var query = db.StkItemDbSet.Where(n => n.ITEMNM == changedText &&
