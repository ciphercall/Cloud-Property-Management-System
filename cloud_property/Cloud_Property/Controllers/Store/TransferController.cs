    ...
    public class TransferController : AppController
    {
        ...
        public ActionResult IndexPost(PageModel model, string command)
        {
            if (command == "Add")
            {
                ...
                var checkPermission = from role in db.AslRoleDbSet
                                      where role.USERID == loggedUserID && role.CONTROLLERNAME == "Transfer" && role.ACTIONNAME == "Index"
                ...
                if (model.StkTrans.STOREFR == null && model.StkTrans.STORETO == null)
                {
                    ViewBag.errorStoreName_StoreFor = "Please select a store name!";
                    ViewBag.errorStoreName_StoreTo = "Please select a store name!";
                    return View("Index");
                }
                else if (model.StkTrans.STOREFR == null)
                {
                    ViewBag.errorStoreName_StoreFor = "Please select a store name!";
                    return View("Index");
                }
                else if (model.StkTrans.STORETO == null)
                {
                    ViewBag.errorStoreName_StoreTo = "Please select a store name!";
                    return View("Index");
                }
                ...
                model.StkTrans.TRANSTP = "ITRF";


                var res = db.StkTransDbSet.Where(a => a.TRANSNO == model.StkTrans.TRANSNO && a.TRANSTP == "ITRF" && a.TRANSYY == model.StkTrans.TRANSYY && a.COMPID == compid).Count(a => a.ITEMID == model.StkTrans.ITEMID) == 0;

                if (res == true)
                {
                    var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSTP == "ITRF" && x.TRANSYY == model.StkTrans.TRANSYY && x.COMPID == compid)
                                  .Max(o => o.ITEMSL);
                    var transno_Max = db.StkTransDbSet.Where(x => x.TRANSYY == model.StkTrans.TRANSYY && x.TRANSTP == "ITRF" && x.COMPID == compid)
                        .Max(s => s.TRANSNO);
                    ...
                    if (model.StkTrans.TRANSNO == null)
                    {
                        ...
                        TempData["STOREFR"] = model.StkTrans.STOREFR;
                        TempData["STORETO"] = model.StkTrans.STORETO;
                        TempData["transdate"] = model.StkTrans.TRANSDT;
                ...
                else
                {
                    var result = (from n in db.StkTransDbSet
                                  where n.TRANSNO == model.StkTrans.TRANSNO &&
                                        ...
                                        n.TRANSYY == model.StkTrans.TRANSYY && n.TRANSTP == "ITRF"
                                  select new
                                  {
            ...
            else if (command == "Save")
            {
                ...
                model.StkTransmst.TRANSTP = "ITRF";
                model.StkTransmst.STOREFR = model.StkTrans.STOREFR;
                model.StkTransmst.STORETO = model.StkTrans.STORETO;
        ...
        public ActionResult OrderDelete(Int64 tid, Int64 orderNo, DateTime Date, Int64 Year, Int64 itemsl, PageModel model)
        {
            ...
            var result = (from t in db.StkTransDbSet
                          where t.COMPID == compid && t.TRANSNO == orderNo && t.TRANSYY == Year && t.TRANSTP == "ITRF"
                          select new { t.TRANSYY, t.STOREFR, t.STORETO, t.PSID, t.TRANSDT }
             ).Distinct().ToList();
            ...
            foreach (var n in result)
            {
                ...
                model.StkTrans.STOREFR = n.STOREFR;
                model.StkTrans.STORETO = n.STORETO;
                TempData["STOREFR"] = model.StkTrans.STOREFR;
                TempData["STORETO"] = model.StkTrans.STORETO;
            ...
            {
                var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSYY == Year && x.TRANSTP == "ITRF" && x.COMPID == compid)
                            .Max(o => o.ITEMSL);
        ...
        public ActionResult EditOrder(PageModel model, string command)
        {
            if (command == "Add")
            {
                ...
                var res = db.StkTransDbSet.Where(a => a.TRANSDT == model.StkTrans.TRANSDT && a.TRANSNO == model.StkTrans.TRANSNO && a.TRANSTP == "ITRF" && a.COMPID == compid).Count(a => a.ITEMID == model.StkTrans.ITEMID) == 0;
                if (res == true)
                {
                    //Permission Check
                    Int64 loggedUserID = Convert.ToInt64(System.Web.HttpContext.Current.Session["loggedUserID"]);
                    ...
                    var get_TRANSDT_TRANSYY = (from m in db.StkTransDbSet
                                               where m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSTP == "ITRF"
                    ...
                    var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSTP == "ITRF" && x.COMPID == compid)
                               .Max(o => o.ITEMSL);
                    var transno_Max = db.StkTransDbSet.Where(x => x.TRANSYY == model.StkTrans.TRANSYY && x.TRANSTP == "ITRF" && x.COMPID == compid)
                        .Max(s => s.TRANSNO);
                    ...
                    else
                    {
                        ...
                        //Remarks pass the value.
                        var transMst = (from m in db.StkTransmstDbSet
                                        where m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSDT == model.StkTrans.TRANSDT && m.TRANSTP == "ITRF"
                                        select new { m.STK_TRANSMST_ID, m.REMARKS }).ToList();
                ...
                else
                {
                    var result = (from n in db.StkTransDbSet
                                  where n.TRANSNO == model.StkTrans.TRANSNO &&
                                        n.COMPID == compid &&
                                        n.ITEMID == model.StkTrans.ITEMID &&
                                        n.TRANSDT == model.StkTrans.TRANSDT && n.TRANSTP == "ITRF"
                                  select new
                                  {
                                      transPID = n.STK_TRANS_ID,
                                      sl = n.ITEMSL,
            ...
            else if (command == "Save")
            {
                ...
                var findTransNO = (from n in db.StkTransmstDbSet
                                   where n.TRANSNO == model.StkTrans.TRANSNO && n.COMPID == model.StkTrans.COMPID &&
                          n.TRANSYY == model.StkTransmst.TRANSYY && n.TRANSTP == "ITRF"
                                   select n).ToList();
                if (findTransNO.Count != 0)
                {
                    foreach (STK_TRANSMST StkTransmst in findTransNO)
                    {
                        ...
                        StkTransmst.TRANSTP = "ITRF";

                        StkTransmst.STOREFR = model.StkTrans.STOREFR;
                        StkTransmst.STORETO = model.StkTrans.STORETO;
            ...
            else if (command == "search")
            {
                ...
                var transMst = (from m in db.StkTransmstDbSet
                                where m.TRANSDT == model.StkTrans.TRANSDT && m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSTP == "ITRF"
                                select new { m.STK_TRANSMST_ID, m.TOTAMT, m.DISCRTG, m.DISCAMTG, m.TOTGROSS, m.VATRT, m.VATAMT, m.OTCAMT, m.TOTNET, m.CASHAMT, m.CREDITAMT, m.REMARKS, m.STOREFR, m.STORETO, m.PSID }).ToList();
                ...
                if (transMst != null)
                {
                    foreach (var a in transMst)
                    {
                        ...
                        model.StkTrans.STOREFR = a.STOREFR;
                        model.StkTrans.STORETO = a.STORETO;
                ...
                var stk_Trans = (from m in db.StkTransDbSet
                                 where m.TRANSDT == model.StkTrans.TRANSDT && m.COMPID == compid && m.TRANSNO == model.StkTrans.TRANSNO && m.TRANSTP == "ITRF"
                                 select new { m.STOREFR, m.STORETO, m.PSID, m.TRANSYY }).ToList();
                foreach (var s in stk_Trans)
                {
                    model.StkTrans.STOREFR = s.STOREFR;
                    model.StkTrans.STORETO = s.STORETO;
        ...
        public ActionResult EditOrderDelete(Int64 tid, Int64 orderNo, DateTime Date, Int64 Year, Int64 itemsl, PageModel model)
        {
            ...
            var result = (from t in db.StkTransDbSet
                          where t.COMPID == compid && t.TRANSNO == orderNo && t.TRANSDT == Date && t.TRANSYY == Year && t.TRANSTP == "ITRF"
                          select t).ToList();
            //Minimum RmsTrans Table MemoNO delete then RmsTransMst tabel data delete(key-> memoNO, compid)
            if (result.Count == 0)
            {
                var searchRmsTransMst = (from n in db.StkTransmstDbSet where n.COMPID == compid && n.TRANSNO == orderNo && n.TRANSDT == Date && n.TRANSYY == Year && n.TRANSTP == "ITRF" select n).ToList();
            ...
            //Remarks pass the value.
            var transMst = (from m in db.StkTransmstDbSet
                            where m.COMPID == compid && m.TRANSNO == orderNo && m.TRANSDT == Date && m.TRANSYY == Year && m.TRANSTP == "ITRF"
                            select new { m.STK_TRANSMST_ID, m.REMARKS }).ToList();
            if (transMst.Count != 0)
            {
                foreach (var a in transMst)
            ...
            var stk_Trans = (from m in db.StkTransDbSet
                             where m.COMPID == compid && m.TRANSNO == orderNo && m.TRANSDT == Date && m.TRANSYY == Year && m.TRANSTP == "ITRF"
                             select new { m.STOREFR, m.STORETO }).ToList();
            foreach (var s in stk_Trans)
            {
                model.StkTrans.STOREFR = s.STOREFR;
                model.StkTrans.STORETO = s.STORETO;
            ...
            var sid = db.StkTransDbSet.Where(x => x.TRANSNO == model.StkTrans.TRANSNO && x.TRANSDT == Date && x.COMPID == compid && x.TRANSYY == Year && x.TRANSTP == "ITRF").Max(o => o.ITEMSL);
            model.StkTrans.TRANSNO = orderNo;
            model.StkTrans.ITEMSL = sid;
            model.StkTrans.TRANSDT = Date;
            model.StkTrans.TRANSYY = Year;
        ...
        public JsonResult DateChanged(string theDate)
        {
            ...
            var transres = (from n in db.StkTransDbSet
                            where n.TRANSDT == dd && n.COMPID == compid && n.TRANSTP == "ITRF"
                            select new
                            {
                                n.TRANSNO
                            }
